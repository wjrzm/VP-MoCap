...
import torch
import numpy as np
import lib.initial_trans.transpose.transmath as M
import time
...
class ParametricModel:
    ...
    """
    def __init__(self, official_model_file: str, scale = 1.0, use_pose_blendshape=False, device=torch.device('cpu')):
        r"""
    ...
    def get_zero_pose_joint_and_vertex(self, shape: torch.Tensor = None):
        ...
        if shape is None:
            ...
            j_scaled, v_scaled = j * self.scale, v * self.scale
        else:
    ...
    def view_mesh(self, vertex_list: list, bg_img=None, fps=60, distance_between_subjects=0.8):
        r"""
        ...
        :param distance_between_subjects: Distance in meters between subjects. 0.2 for hand and 0.8 for body is good.
        """
        from lib.utils.visualize_o3d import render_sequence_3d, vis_mesh
        v_list, f_list = [], []
        f = self.face.copy()
        ...
        faces = np.concatenate(f_list)
        if verts.shape[0] > 1:
            render_sequence_3d(verts, faces, 'result_%s.mp4' % timestr, bg_img=bg_img, visible=True,need_norm=False)
        else:
            vis_mesh(verts[0], faces)

    def get_joint_vertex(self, pose: torch.Tensor, shape: torch.Tensor = None, tran: torch.Tensor = None):
        r"""
        Get mesh vertex positions from poses, shapes, and translations. (torch, batch)

        :param pose: Joint local rotation tensor in shape [batch_size, *] that can reshape to
                     [batch_size, num_joint, 3, 3] (rotation matrices).
        :param shape: Tensor for model shapes that can expand to [batch_size, 10]. Use None for the mean(zero) shape.
        :param tran: Root position tensor in shape [batch_size, 3]. Use None for the zero positions.
        :return: Mesh vertex position in [batch_size, num_vertex, 3].
        """
        pose = pose.view(-1, len(self.parent), 3, 3)
        shape = shape.view(-1, 10) if shape is not None else None
        tran = tran.view(-1, 3) if tran is not None else None

        result = self.forward_kinematics(pose, shape, tran, calc_mesh=True)

        return result[1], result[2]

    def view_motion(self, pose_list: list, beta_list: list = None,tran_list: list = None, bg_img=None, fps=30, distance_between_subjects=0):
        r"""
        ...
        :param beta_list: List in length [num_subject] of tensors that can all reshape to [num_frame, 10].
        ...
        for i in range(len(pose_list)):
            ...
            # pose[:,0] = torch.tensor([[-1,0,0],[0,1,0],[0,0,-1]],dtype=torch.float32) @ pose[:,0]
            
            # tran = tran_list[i].view(-1, 3) - tran_list[i].view(-1, 3)[:1] if tran_list else None
            beta = beta_list[i].view(-1, 10) if beta_list else None
            tran = tran_list[i].view(-1, 3) if tran_list else None
            verts.append(self.forward_kinematics(pose, shape=beta, tran=tran, calc_mesh=True)[2])
        # self.view_mesh(verts, bg_img ,fps, distance_between_subjects=distance_between_subjects)
        return verts[0]
