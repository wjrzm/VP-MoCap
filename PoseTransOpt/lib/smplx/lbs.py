...
def lbs(
    ...
    # # Root joint is aligned to zero.
    # # Transpose use this method
    # verts = verts - J_transformed[:,:1]
    # J_transformed = J_transformed - J_transformed[:,:1]

    # add scale in the end
    # use .clone for optimize
    if torch.is_tensor(scale) is True:
        verts = verts * scale.clone()
        J_transformed = J_transformed * scale.clone()
    else:
        verts = verts * scale
        J_transformed = J_transformed * scale
